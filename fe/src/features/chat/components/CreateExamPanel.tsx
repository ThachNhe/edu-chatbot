...
export function CreateExamPanel({
    ...
    return (
        <div className="mx-3 mb-2 rounded-xl border border-[#bfdbfe] bg-[#eff6ff] p-4 shadow-sm">
            {/* Header */}
            ...
            <div className="mb-3">
                ...
                <div className="flex flex-col gap-1.5">
                    {attachedFiles.map((file, idx) => (
                        <div
                            key={idx}
                            className="flex items-center gap-2 rounded-lg border border-[#bfdbfe] bg-white px-3 py-2"
                        >
                    ...
                    {canAddMore && (
                        <button
                            ...
                            className="flex w-full items-center justify-center gap-2 rounded-lg border-2 border-dashed border-[#bfdbfe] bg-white py-2 text-[12.5px] font-semibold text-[#1a56db] transition hover:border-[#1a56db] hover:bg-white/70"
                        >
                            <PlusCircle size={14} />
            ...
            <div className="mb-3">
                ...
                <input
                    ...
                    disabled={isLoading}
                    className="w-full rounded-lg border border-[#e2e8f0] bg-white px-3 py-2 text-[12.5px] text-[#1e293b] outline-none placeholder:text-[#94a3b8] focus:border-[#1a56db] disabled:opacity-50"
                />
            </div>
                ...
                <div className="flex flex-wrap gap-1.5">
                    {QUESTION_COUNT_OPTIONS.map((n) => (
                        <button
                            ...
                            disabled={isLoading}
                            className={`rounded-lg px-3 py-1.5 text-[12px] font-semibold transition ${questionCount === n
                ...
                <div className="flex flex-wrap gap-1.5">
                    {DIFFICULTY_OPTIONS.map((opt) => (
                        <button
                            ...
                            disabled={isLoading}
                            className={`rounded-lg px-3 py-1.5 text-[12px] font-semibold transition ${difficulty === opt.value
                                    ? 'bg-[#1a56db] text-white shadow-sm'
                                    : 'border border-[#e2e8f0] bg-white text-[#475569] hover:border-[#1a56db] hover:text-[#1a56db]'
                                } disabled:opacity-50`}
                        >
            ...
            <div className="mb-4">
                ...
                <input
                    ...
                    disabled={isLoading}
                    className="w-24 rounded-lg border border-[#e2e8f0] bg-white px-3 py-2 text-[12.5px] text-[#1e293b] outline-none focus:border-[#1a56db] disabled:opacity-50"
                />
            </div>
            ...
            <div className="flex items-center gap-2">
                <button
                    ...
                    disabled={!canSubmit}
                    className="flex items-center gap-1.5 rounded-lg bg-[#1a56db] px-4 py-2 text-[12.5px] font-bold text-white shadow-sm transition hover:bg-[#1d4ed8] disabled:cursor-not-allowed disabled:opacity-40"
                >
                    {isLoading ? (
                        <>
                ...
                <button
                    onClick={onCancel}
                    disabled={isLoading}
                    className="rounded-lg border border-[#e2e8f0] bg-white px-4 py-2 text-[12.5px] font-semibold text-[#475569] transition hover:border-[#1a56db] hover:text-[#1a56db] disabled:opacity-40"
                >
                    Hủy
                </button>
            </div>
